...
describe AnnotateModels do
  ...
  describe ".set_defaults" do
    subject do
      Annotated::Helpers.true?(ENV["show_complete_foreign_keys"])
    end
    ...
    context 'when default value of "show_complete_foreign_keys" is set' do
      before do
        Annotated.set_defaults("show_complete_foreign_keys" => "true")
      end

      after do
        Annotated.instance_variable_set(:@has_set_defaults, false)
      end
  ...
  describe "annotating a file" do
    before :each do
      ...
      Annotated::Helpers.reset_options(Annotated::Constants::ALL_ANNOTATE_OPTIONS)
    end
    ...
    def annotate_one_file(options = {})
      Annotated.set_defaults(options)
      options = Annotated.setup_options(options)
      ...
      Annotated.instance_variable_set(:@has_set_defaults, false)
      Annotated::Constants::POSITION_OPTIONS.each { |key| ENV[key.to_s] = "" }
      Annotated::Constants::FLAG_OPTIONS.each { |key| ENV[key.to_s] = "" }
      Annotated::Constants::PATH_OPTIONS.each { |key| ENV[key.to_s] = "" }
    end
    ...
    describe "if a file can't be annotated" do
      ...
      it "displays just the error message with trace disabled (default)" do
        ...
        expect { AnnotateModels.do_annotations model_dir: @model_dir, is_rake: true }.not_to output(a_string_including("/spec/annotated/annotate_models_spec.rb:")).to_stderr
      end
      ...
      it "displays the error message and stacktrace with trace enabled" do
        ...
        expect { AnnotateModels.do_annotations model_dir: @model_dir, is_rake: true, trace: true }.to output(a_string_including("/spec/lib/annotated/annotate_models_spec.rb:")).to_stderr
      end
    end
